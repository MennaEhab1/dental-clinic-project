...
export function DashboardLayout({ children, role }: DashboardLayoutProps) {
  ...
  useEffect(() => {
    ...
    patientService
      ...
      .catch((error) => {
        console.warn("[DashboardLayout] Failed to refresh patient profile", error);
      });

    const handleProfileUpdated = (event: Event) => {
      const profile = (event as CustomEvent<Patient>).detail;
      const userId = String(user.id || user.userId || "");
      const userEmail = String(user.email || "");
      const isSameUser =
        (!!userId && !!profile.id && userId === profile.id) ||
        (!!userEmail && !!profile.email && userEmail === profile.email);
