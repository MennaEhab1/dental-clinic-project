...
export default function AdminAppointments() {
  ...
  // ✅ Fetch slots when doctor and date are both selected
  useEffect(() => {
    ...
    const fetchSlots = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `${BASE_URL}/api/DoctorSchedule/available-slots?doctorId=${createForm.doctorId}&date=${dateIso}`,
          { headers: { Authorization: `Bearer ${getToken()}` } }
        );
        if (!res.ok) throw new Error("Failed to fetch slots");
        const data = await res.json();
        const slots = Array.isArray(data) ? data : (data?.value ?? []);
        setAvailableSlots(slots.filter((s: AvailableSlot) => s.isAvailable));
      } catch {
        setAvailableSlots([]);
      } finally {
    ...
    void fetchSlots();
  }, [createForm.doctorId, createForm.date]);
  ...
  const getPatientDisplayName = (apt: Appointment) => {
    ...
    const lastName = apt.patient?.lastName?.trim() || "";
    const fullName = `${firstName} ${lastName}`.trim();
    return fullName || apt.patient?.email || apt.patient?.id || `Patient #${apt.patientId || "—"}`;
  };

  const getPatientAvatarFallback = (apt: Appointment) => {
    const name = getPatientDisplayName(apt);
    return name.split(/\s+/).filter(Boolean).map((part) => part[0].toUpperCase()).slice(0, 2).join("") || "P";
  };
  ...
  const validateCreateForm = () => {
    ...
    if (!createForm.patientId) errs.patientId = "Patient is required";
    if (!createForm.date) errs.date = "Date is required";
    if (!createForm.startTime) errs.startTime = "Please select an available time slot";
    if (isNaN(Number(createForm.amount)) || Number(createForm.amount) < 0)
      errs.amount = "Valid amount is required";
    setCreateErrors(errs);
    return Object.keys(errs).length === 0;
  ...
  const handleCreateAppointment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const patientIdNum = Number(createForm.patientId);
      if (isNaN(doctorIdNum) || isNaN(patientIdNum)) {
        throw new Error("Invalid doctor or patient ID — please re-select from the dropdowns");
      }
      const dateIso = `${createForm.date}T00:00:00Z`;
      const startTime = createForm.startTime.length === 5
      ...
      setCreateOpen(false);
      setCreateForm({ doctorId: "", patientId: "", date: "", startTime: "", amount: "0", paymentMethod: "Cash" });
      setCreateErrors({});
      setAvailableSlots([]);
    } catch (error) {
    ...
    <DashboardLayout role="admin">
      <div className="space-y-6">
        ...
        >
          ...
          <Button className="gradient-bg border-0 gap-2 shrink-0" onClick={() => setCreateOpen(true)}>
            <Plus className="w-4 h-4" />
            New Appointment
          </Button>
        </motion.div>
                    ...
                    <tr key={apt.id} className="border-b border-border last:border-0 hover:bg-muted/30 transition-colors">
                      <td className="py-3">
                        <div className="flex items-center gap-2">
                          <Avatar className="h-8 w-8">
                            <AvatarImage src={apt.patient?.avatar} />
                            <AvatarFallback className="text-xs">{getPatientAvatarFallback(apt)}</AvatarFallback>
                          </Avatar>
                          <span className="text-foreground font-medium">{getPatientDisplayName(apt)}</span>
                        </div>
                      </td>
                      <td className="py-3 text-muted-foreground">{getDoctorDisplayName(apt)}</td>
                      ...
                      <td className="py-3">
                        <div className="flex gap-2">
                          <Button size="sm" variant="outline" onClick={() => openDetails(apt)} disabled={isUpdating || isLoadingDetails}>
                            View Details
                          </Button>
                          {apt.status !== "cancelled" && apt.status !== "complete" && (
                            <>
                              {apt.status === "upcoming" && (
                                <Button size="sm" className="gap-1" onClick={() => handleStatusChange(apt.id, "complete")} disabled={isUpdating}>
                                  <Check className="w-3 h-3" /> Complete
                                </Button>
                              )}
                              <Button size="sm" variant="destructive" className="gap-1" onClick={() => handleCancelAppointment(apt.id)} disabled={isUpdating}>
      ...
      <Dialog open={createOpen} onOpenChange={(open) => {
        ...
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>New Appointment</DialogTitle>
            <DialogDescription>Book an appointment on behalf of a patient.</DialogDescription>
          </DialogHeader>
          <form onSubmit={handleCreateAppointment} className="space-y-4">
            ...
            <div>
              ...
              <Select
                value={createForm.doctorId}
                onValueChange={(v) => setCreateForm((p) => ({ ...p, doctorId: v, startTime: "" }))}
              >
                <SelectTrigger id="ca-doctor" className={`mt-1 ${createErrors.doctorId ? "border-destructive" : ""}`}>
                  <SelectValue placeholder="Select doctor" />
                </SelectTrigger>
                <SelectContent>
                  {doctors.map((d) => (
              ...
              {createErrors.doctorId && <p className="text-xs text-destructive mt-1">{createErrors.doctorId}</p>}
            </div>
            ...
            <div>
              ...
              {patients.length > 0 ? (
                <Select
                  value={createForm.patientId}
                  onValueChange={(v) => setCreateForm((p) => ({ ...p, patientId: v }))}
                >
                  <SelectTrigger id="ca-patient" className={`mt-1 ${createErrors.patientId ? "border-destructive" : ""}`}>
                    <SelectValue placeholder="Select patient" />
                  </SelectTrigger>
                  <SelectContent>
                    {patients.map((pt) => (
                      <SelectItem key={pt.id} value={pt.id}>
                        {pt.firstName} {pt.lastName}{pt.email ? ` — ${pt.email}` : ""}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              ...
              {createErrors.patientId && <p className="text-xs text-destructive mt-1">{createErrors.patientId}</p>}
            </div>
            ...
            <div>
              ...
              <Input
                ...
                type="date"
                value={createForm.date}
                onChange={(e) => setCreateForm((p) => ({ ...p, date: e.target.value, startTime: "" }))}
                className={`mt-1 ${createErrors.date ? "border-destructive" : ""}`}
              />
              {createErrors.date && <p className="text-xs text-destructive mt-1">{createErrors.date}</p>}
            </div>

            {/* ✅ Available Time Slots */}
            <div>
              <Label htmlFor="ca-slot">Available Time Slots</Label>
              <Select
                value={createForm.startTime}
                onValueChange={(v) => setCreateForm((p) => ({ ...p, startTime: v }))}
                disabled={isLoadingSlots || (!createForm.doctorId || !createForm.date)}
              >
                <SelectTrigger id="ca-slot" className={`mt-1 ${createErrors.startTime ? "border-destructive" : ""}`}>
                ...
                <SelectContent>
                  {isLoadingSlots ? (
                    ...
                    availableSlots.map((slot, idx) => (
                      <SelectItem key={idx} value={slot.startTime}>
              ...
              {createErrors.startTime && <p className="text-xs text-destructive mt-1">{createErrors.startTime}</p>}
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Input
                  ...
                  className={`mt-1 ${createErrors.amount ? "border-destructive" : ""}`}
                />
                {createErrors.amount && <p className="text-xs text-destructive mt-1">{createErrors.amount}</p>}
              </div>
              <div>
                <Label htmlFor="ca-payment">Payment</Label>
                <Select
                  value={createForm.paymentMethod}
                  onValueChange={(v) => setCreateForm((p) => ({ ...p, paymentMethod: v as "Cash" | "Visa" }))}
                >
                  <SelectTrigger id="ca-payment" className="mt-1">
                    <SelectValue />
                  </SelectTrigger>
            ...
            <div className="flex gap-2 pt-1">
              <Button type="button" variant="outline" className="flex-1" onClick={() => setCreateOpen(false)} disabled={isCreating}>
                Cancel
              </Button>
              <Button type="submit" className="flex-1 gradient-bg border-0" disabled={isCreating}>
                {isCreating ? (
